 ...
 */
public class SimplePhraseAdapter extends AbstractPlayerAdapter {

    MemorizeDBHelper mDBHelper;
    BaseBookData mUpdatedBook;

    public SimplePhraseAdapter(Context context) {
        super(context);
    ...
    public void onItemList(ArrayList<RawData> dataList) {
        ...
        if(mCurrentReader.getReaderType() == Constants.ReaderType.DB) {
            ...
            for(RawData data : dataList) {
                SimplePhraseData convertedData = new SimplePhraseData();
    ...
    public void write(SimplePhraseData data) {
